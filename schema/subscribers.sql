...
SELECT count(id) AS total FROM subscribers WHERE active = true;

CREATE VIEW total_inactive_users AS
SELECT count(id) AS total FROM subscribers WHERE active = false;
